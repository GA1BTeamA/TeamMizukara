			...
			if (m_p_hit_line_spri3->GetHitData()[i]->GetElement() == 0)
			{
				if (m_move1 >= 120)
				{
					...
					else if (m_ani_time4 > 300)//ホースを付け終わり再び主人公静止
					{
						...
						if (g_SceneNumber == RESULT_MAIN&&m_vy == 0.0f&&g_TankRemaining < 35)
						{
							Draw::Draw2D(118, m_wave_x + ground3->GetScroll() - 50 + m_move1, 220, 1, 1);
						}
						else if (g_SceneNumber == RESULT_MAIN&&m_vy == 0.0f)
						{
							Draw::Draw2D(119, m_wave_x + ground3->GetScroll() - 50 + m_move1, 220, 1, 1);
						}
						else if (m_sy <= 200)
						{
