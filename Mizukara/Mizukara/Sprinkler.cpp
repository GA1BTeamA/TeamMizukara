			...
			if (m_p_hit_line_spri->GetHitData()[i]->GetElement() == 0)
			{
				if (m_move1 >= 120)
				{
					...
					else if (m_ani_time4 > 300)//ホースを付け終わり再び主人公静止
					{
						...
						if (g_SceneNumber == RESULT_MAIN&&m_vy == 0.0f&&g_TankRemaining < 37)
						{
						...
						if (m_ani_time4 >= 460)
						{
							...
							if (tank->GetWater_Remaining() > 0)
							{
								...
								{
									Draw::Draw2D(51, m_wave_x + ground->GetScroll() + 60 + m_move1, m_wave_y-4, 1, 1);
								}
								else if (m_move2 >= 11 && m_move2 < 20)
								{
									Draw::Draw2D(51, m_wave_x + ground->GetScroll() + 97 + m_move1, m_wave_y - 6, -1, 1);
								}
								else
								{
									m_move2 = 0;
