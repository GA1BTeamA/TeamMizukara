...
void CCherClass::CreateCharTex(wchar_t c, HDC hdc, TEXTMETRIC TM)
{
	...
	desc.MipLevels = 1;								
	desc.ArraySize = 1;								
	desc.Format	   = DXGI_FORMAT_R8G8B8A8_UNORM;	//テクスチャフォーマットR8G8B8A8の32bit
...
//文字描画
void CFontTex::StrDraw(wchar_t* str, float x, float y, float s, float r, float g, float b, float a)
{
	//文字列を登録
	CreateStrTex(str);
...
//文字列を元に文字テクスチャを作成
void CFontTex::CreateStrTex(wchar_t* str)
{
